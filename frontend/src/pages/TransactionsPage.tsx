  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
        <Card className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 border-green-500/30 hover:scale-[1.02] transition-transform">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-green-500/20 rounded-xl flex items-center justify-center">
              <TrendingUp className="w-7 h-7 text-green-400" />
            </div>
            <div>
              <p className="text-white/90 text-sm mb-1">{t('transactions.totalTopup')}</p>
              <p className="text-3xl font-bold text-green-400">{formatCurrency(totalTopup, true)}</p>
            </div>
          </div>
        </Card>

        <Card className="bg-gradient-to-br from-cyan-500/10 to-teal-500/10 border-cyan-500/30 hover:scale-[1.02] transition-transform">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-cyan-500/20 rounded-xl flex items-center justify-center">
              <Calendar className="w-7 h-7 text-cyan-400" />
            </div>
            <div>
              <p className="text-white/90 text-sm mb-1">{t('transactions.thisMonth')}</p>
      ...
      {/* Transactions Table */}
      <Card title={t('transactions.history')}>
        {transactions.length === 0 ? (
          <div className="text-center py-12">
            <ArrowDownCircle className="w-16 h-16 mx-auto mb-4 text-white/60" />
            <p className="text-white">{t('transactions.noHistory')}</p>
          </div>
        ) : (
          <>
            {/* Desktop Table */}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-800">
                    ...
                    <th className="text-right py-4 px-4 text-white font-semibold text-sm">AMOUNT</th>
              ...
              {transactions.map((transaction, idx) => (
                <div key={transaction._id} className="bg-gray-950/50 rounded-xl p-4 border border-gray-800">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-white/90 text-xs">{formatDateShort(transaction.createdAt)}</span>
                  </div>
