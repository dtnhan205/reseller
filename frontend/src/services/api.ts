...
api.interceptors.request.use(
  (config) => {
    // Không gửi token cho các endpoint login và register
...
export const adminApi = {
  ...
  createCategory: async (name: string, image?: string, order?: number, status?: 'active' | 'inactive'): Promise<Category> => {
    const res = await api.post('/admin/categories', { name, image, order, status });
    // Backend returns { category }, so we need to extract it
    return res.data.category || res.data;
  },
  updateCategory: async (id: string, data: { name?: string; image?: string; order?: number; status?: 'active' | 'inactive' }): Promise<Category> => {
